...
*   - malloc(): 지정한 크기의 메모리를 할당하고, 초기화 되지 않은 메모리 변환
*   - calloc(): 지정한 크기의 메모릐를 할당하고, 0으로 초기화
...
*  4. 메모리를 효율족으로 관리하고 할 대
*   - 필요할 대 메모리를 할당하고 해제하여 메모리 낭비를 줄임
*   - 정적배열과 동적배열의 차이
...
#include <stdio.h>
#include <stdlib.h> // malloc, free 함수 사용을 위한 헤더 파일일
